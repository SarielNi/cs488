...
void A0::guiLogic()
{
	...
	ImGui::Begin("Shape Properties", &showDebugWindow, ImVec2(100,100), opacity,
		...
		ImGui::SliderFloat("Red Channel", &m_shape_color.r, 0.0f, 1.0f);		//red channel slider
		ImGui::SliderFloat("Green Channel", &m_shape_color.g, 0.0f, 1.0f);		//green channel slider
		ImGui::SliderFloat("Blue Channel", &m_shape_color.b, 0.0f, 1.0f);		//blue channel slider
		ImGui::SliderFloat("Rotate Channel", &m_shape_rotation, 0.0f, 2*PI);	//rotate channel slider
...
//----------------------------------------------------------------------------------------
/*
 * Reset the triangle to initial value
 */
void A0::resetTriangle()
{
	//reset color
...
bool A0::keyInputEvent(int key, int action, int mods) {
	...
	if (action == GLFW_PRESS) {
		if (key == GLFW_KEY_EQUAL) {
			...
			if ( m_shape_size == 10.0f ){
				//the maximum size if 10.0f
		...
		if (key == GLFW_KEY_MINUS) {
			...
			if ( m_shape_size <= 0.1f ){
				//the minimum size is 0.5f
				cout << "Reach the Minimum size" << endl;
			}else if ( m_shape_size <= 1.0f) {
				m_shape_size -= 0.1f;
			}else{
				m_shape_size -= 0.5f;
			}
		...
		if (key == GLFW_KEY_R) {
			cout << "Reset the triangle" << endl;
			//reset the scale, color, position and rotation
			resetTriangle();
			eventHandled = true;
		}
	}
